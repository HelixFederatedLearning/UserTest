...
import express from 'express';
import multer from 'multer';
import UploadLog from '../models/UploadLog.js';           // default import ✅
import { requireAuth } from '../middleware/auth.js';
import { preprocessToTensor } from '../utils/preprocess.js';
import { ensureSessionLoaded, runInference } from '../services/onnx.js';
...
// Memory storage; allow up to 2 images, 20MB each
const upload = multer({
  storage: multer.memoryStorage(),
...
function postprocess(outputTensor) {
  // outputTensor may be: ort.Tensor with .data, or a raw float array
  const data = outputTensor?.data ?? outputTensor;
  const logits = Array.from(data);
  const probs = softmax(logits);
...
router.post('/upload', requireAuth, upload.array('images', 2), async (req, res, next) => {
  try {
    await ensureSessionLoaded(); // ok if model isn't configured; service handles it
    const consent = String(req.body?.consent || '').toLowerCase() === 'true';
    const files = req.files || [];
    if (!files.length) return res.status(400).json({ error: 'No image files provided' });

    // preprocess first image (extend to batch if needed)
    const tensor = await preprocessToTensor(files[0].buffer);

    // run inference
    const output = await runInference(tensor); // should return Float32Array/ort.Tensor
    const { predictions, top } = postprocess(output);

    // save log in SAME collection used by client-log
    const userId = req.user.id || req.user._id;
    await UploadLog.create({
      userId,
      filenames: files.map(f => f.originalname).slice(0, 2),
      consent,
      predictions,
      source: 'server',
    });

    res.json({ ok: true, top, predictions });
  } catch (err) {
    next(err);
  }
});
